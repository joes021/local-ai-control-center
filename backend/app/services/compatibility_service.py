...
def run_compatibility_check(
    *,
    ...
    resolved_model = _resolve_compatibility_model(
        ...
        model=model,
        local_qwen_home=local_qwen_home,
    )
    if not resolved_model:
        return _not_checked_result("Model nije pronadjen za compatibility proveru.")

    system_info = detect_local_system_info(local_qwen_home=local_qwen_home)
...
def apply_compatibility_action(
    *,
    ...
    result = _apply_action(action, local_qwen_home=local_qwen_home)
...
def calculate_compatibility(model: dict[str, object], *, system_info: dict[str, object]) -> dict[str, object]:
    ...
    result = {
        ...
        "reasoning": _build_reasoning(normalized_model, normalized_system, estimated, fit_status, speed_status),
        "memoryBudget": {
...
def check_model_compatibility(
    *,
    ...
    from backend.app.services.browser_catalog_service import update_model_fit_status
    ...
    update_model_fit_status(model_id, result, local_qwen_home=local_qwen_home)
    return result


def detect_local_system_info(*, local_qwen_home: Path | None = None) -> dict[str, object]:
    ...
    llama = settings.get("llama") if isinstance(settings.get("llama"), dict) else {}
    turbo_config = load_turboquant_config(local_qwen_home=home)
    selected_runtime = str(install_state.get("selectedRuntime", "") or "")
    turbo_server = str(install_state.get("turboServerExe", "") or "")
    return {
        "ramGiB": detect_ram_gib(),
        "vramGiB": detect_vram_gib(),
        "turboQuantAvailable": bool(turbo_server) or selected_runtime.lower() == "turboquant",
...
def _resolve_compatibility_model(
    *,
    ...
    local_qwen_home: Path | None,
) -> dict[str, object] | None:
    if model:
        return _normalize_model(model)
    if not catalog_model_id:
        return None
    from backend.app.services.browser_catalog_service import get_catalog_model

    payload = get_catalog_model(catalog_model_id, local_qwen_home=local_qwen_home)
    if payload:
...
def _normalize_model(model: dict[str, object]) -> dict[str, object]:
    ...
    if recommended_vram is None:
        ...
        recommended_vram = (
            round((recommended_gpu_mib or 0) / 1024, 2)
            if recommended_gpu_mib is not None
            else None
        )
...
def _estimate_budget(model: dict[str, object], system: dict[str, object]) -> dict[str, object]:
    ...
    context_window = _as_float(model.get("contextWindow")) or 32768
    default_output = _as_float(model.get("defaultOutputTokens")) or 4096
...
def _build_reasoning(
    ...
    return {
        "vram": (
            (
                f"VRAM je premalo jak: procena trazi oko {estimated['requiredVramGiB']:.1f} GiB, a dostupno je {available_vram:.1f} GiB."
                if available_vram < estimated["requiredVramGiB"]
                else f"VRAM deluje dovoljno jak: procenjeni budzet je oko {estimated['requiredVramGiB']:.1f} GiB naspram dostupnih {available_vram:.1f} GiB."
            )
        ),
        "ram": (
            (
                f"RAM je premalo jak: procena trazi oko {estimated['requiredRamGiB']:.1f} GiB, a dostupno je {available_ram:.1f} GiB."
                if available_ram < estimated["requiredRamGiB"]
                else f"Procenjeni RAM budzet je oko {estimated['requiredRamGiB']:.1f} GiB naspram dostupnih {available_ram:.1f} GiB."
            )
        ),
...
def _build_recommendations(
    ...
    if context > 131072:
        target_context = 131072 if context > 131072 else 65536
        recommendations.append(
            {
                "id": "context-downshift",
                "title": f"Smanji context na {target_context}",
                ...
                "severity": "warn",
                "action": {"kind": "set-context", "value": target_context},
            }
        )

    if output_tokens > 4096:
        target_output = 4096 if output_tokens > 4096 else 2048
        recommendations.append(
            {
                "id": "output-downshift",
                "title": f"Smanji output na {target_output}",
                ...
                "severity": "info",
                "action": {"kind": "set-output", "value": target_output},
            }
        )
...
def _apply_action(action: dict[str, object], *, local_qwen_home: Path | None = None) -> dict[str, object]:
    ...
    if kind == "apply-package":
        ...
        for nested in action.get("actions") or []:
            if not isinstance(nested, dict):
                continue
            latest = _apply_action(nested, local_qwen_home=local_qwen_home)
            if latest.get("status") != "ok":
                return latest
        return latest or _action_result("error", "Compatibility paket nije imao primenljive akcije.")
    if kind == "set-context":
        return _apply_settings_patch({"context": int(action.get("value", 131072) or 131072)})
    if kind == "set-output":
        return _apply_settings_patch({"outputTokens": int(action.get("value", 4096) or 4096)})
    if kind == "set-runtime-preference":
        return _apply_turbo_patch({"runtimePreference": str(action.get("value", "turboquant") or "turboquant")}, summary="TurboQuant runtime preference je sacuvan.")
    if kind == "set-ncmoe":
        return _apply_turbo_patch({"ncmoe": int(action.get("value", 20) or 20)}, summary="TurboQuant ncmoe je sacuvan.")
    if kind == "set-ctk-ctv":
        return _apply_turbo_patch(
            {
                "ctk": str(action.get("ctk", "turbo4") or "turbo4"),
                "ctv": str(action.get("ctv", "turbo3") or "turbo3"),
            },
...
def _apply_settings_patch(patch: dict[str, object]) -> dict[str, object]:
...
def _apply_turbo_patch(patch: dict[str, object], *, summary: str) -> dict[str, object]:
    current = load_turboquant_config()
    current.update(patch)
    save_turboquant_config(current)
